...
export function PhaseTransitionButtons({
  ...
  const handleSubmit = (actionTo: string, e: React.FormEvent<HTMLFormElement>) => {
    ...
    } else if (actionTo === "round2" && r1VotesCount === 0) {
      setConfirmTo(actionTo);
    } else {
        ...
        >
          ...
          >
            {confirmTo === "round1" ? (
              ...
              <>
                <h3 className="text-lg font-semibold text-white">Start Round 2 with no Round 1 votes?</h3>
                <p className="mt-2 text-sm text-[color:var(--color-muted)] leading-relaxed">
                  There are currently <strong className="text-white">0 votes</strong> cast in Round 1.
                </p>
                <p className="mt-2 text-sm text-[color:var(--color-muted)] leading-relaxed">
                  Starting Round 2 now may result in an empty ballot. <span className="font-semibold">Are you sure you want to proceed?</span>
                </p>
              </>
            )}
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              >
                {pending ? "Advancing..." : `Yes, start ${confirmTo === "round1" ? "Round 1" : "Round 2"}`}
              </button>
            </div>
          </div>
        </div>
