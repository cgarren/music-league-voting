...
- **Tailwind v4**. There is **no `tailwind.config.js`**. Theme tokens live in [`src/app/globals.css`](src/app/globals.css) under `@theme inline { ... }`. Add new colors/tokens there, then reference them with the arbitrary-value syntax `bg-[color:var(--color-TOKEN)]` (substitute your token name) or via the `@theme`-generated utility name. **Do not bulk-rewrite the existing `bg-[color:var(...)]`, `border-[color:var(...)]`, or `text-[color:var(...)]` classes to Tailwind's canonical shorthand suggestions.** In this project/Cursor setup, accepting those `suggestCanonicalClasses` warnings caused color utilities to stop applying at runtime, so borders fell back to `currentColor` and purple outlines turned light/white. The workspace setting suppresses that diagnostic; keep the explicit arbitrary color form unless you visually verify a safer token strategy.
