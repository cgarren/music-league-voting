...
- **Round 1 — pick 3.** Signed-in users check exactly three topics they
  would love to vote songs on. Any topic that gets ≥ 1 vote moves forward.
- **Round 2 — spend 10 votes.** Voters distribute exactly 10 stackable
  ...
  them thin — your call.
- **Results.** Top ten by vote total, with submitters revealed.
...
- `topics` — imported from the sheet; `normalized_text` powers duplicate hints.
- `round1_votes` — PK `(user_id, topic_id)`; trigger caps at 3 per user per session.
...
2. Sign in with Google. An **Admin** link appears in the navbar.
...
5. Click **Open Round 1** → voters see `/vote/round1`.
6. When ready, click **Close Round 1 → Open Round 2**.
7. When ready, click **Close Round 2 → Publish Results**.
8. **Archive & Start Fresh** lets you run the next session.
